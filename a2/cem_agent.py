...
class CEMAgent(base_agent.BaseAgent):
    ...
    def _sample_candidates(self, n):
        ...
        candidates = torch.randn([n, self._param_mean.shape[0]], device=self._device) * self._param_std + self._param_mean
    ...
    def _eval_candidates(self, candidates):
        ...
        rets = np.zeros(candidates.shape[0])
        ep_lens = np.zeros(candidates.shape[0])


        for i in range(candidates.shape[0]):
            torch.nn.utils.vector_to_parameters(candidates[i], self._model.parameters())

            result = self._rollout_test(self._eps_per_candidate)
            returns = result['mean_return']
            episode_lengths = result['mean_ep_len']

            rets[i] = returns
            ep_lens[i] = episode_lengths
    ...
    def _compute_new_params(self, params, rets):
        ...
        #rets_tensor = torch.tensor(rets, dtype=params.dtype, device=self._device)

        #weighted_params = torch.matmul(rets_tensor, params)
        #total_returns = rets_tensor.sum()
        #new_mean = weighted_params / total_returns

        #diff_squared = (params - new_mean) ** 2
        #weighted_diff_squared = torch.matmul(rets_tensor, diff_squared)
        #new_std = torch.sqrt(weighted_diff_squared / total_returns)

        new_std, new_mean = torch.std_mean(torch.index_select(params ,dim=0, index=torch.topk(torch.from_numpy(rets), k=int(rets.shape[0] * self._elite_ratio)).indices), dim=0)
