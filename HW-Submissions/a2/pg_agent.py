...
class PGAgent(base_agent.BaseAgent):
    ...
    ''' it seems like i implemented everything just like the guidline, but i did not get any results close to the treshold. please let me know where did i have some mistakes to take care of. thank you. '''
    ...
    def _calc_return(self, r, done):
        ...
        # placeholder
        return_t = torch.zeros_like(r) # requires_grad=True
        
        """ S """
        num_timesteps = r.shape[0]
    
        for t in reversed(range(num_timesteps)):
            if done[t]:
                return_t[t] = r[t]
            else:
                return_t[t] = r[t] + self._discount * return_t[t] # * return_t[t+1]
        """ F """
        return return_t
